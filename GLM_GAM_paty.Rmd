...
```{r ggpairs expected most important variables 1, message=FALSE}
...
To see more details, we made for several categorical variables boxplots.
```{r Boxplots for categorical variables, echo=FALSE, message=FALSE}
...
### LM
Check mean percentage good of all observations and test against amt_income_total.
```{r LM simple model, message=FALSE}
...
Add further variables to the LM and again test against less complex models.
```{r LM - checking variable importance - global F-test, message=FALSE}
...
```{r Data Prep Poisson, include=FALSE, message=FALSE}
...
```{r Visualization number of occurrences per category, message=FALSE}
ggplot(df.barplot, aes(x = status, y = status_counted)) + 
    geom_bar(stat = "identity") +
    geom_text(aes(label = status_counted), vjust = -0.3, size = 3, colour = "black")
```

```{r Visualization mean number of occurrences per category, message=FALSE}
df.poisson %>% 
...
#### Creating a model based on the most Important Variables

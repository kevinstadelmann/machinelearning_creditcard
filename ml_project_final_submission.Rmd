...
We start the graphical analysis by plotting the response variable (i.e. percent_good) against each of the predictors age_years and work_years, however see no clear relationship. We do, however, see a slightly negative relationship with amt_total_income.
...
# GLM Binomial Model
For the GLM Binomial model we use the response variable proportion of good customers, customer_status_good, where we divided the customer status in the data set into good (1) and bad (0) customer count for credit-worthiness. We would expect amt_income_total to have the strongest impact on the response variable, which is related to credit-worthiness. We are also interested in seeing if variables such as age_years, work_years and cnt_children have a significant impact on the target variables.
...
```
The graphical analysis indicates that the predictors have almost no effect on our dependent variable. Some of the variables seem to show interaction.
...
```
Only the predictor amt_income_total seems to play a significant role. The output indicates that if we were to increase this variable by one unit, we could expect an increase of the proportion of good customers also by almost 1.

When interaction between the selected variables is included, it is interesting to see that it plays in some cases a significant role. Also the pairs plot shows interaction between different variables (model not shown).
...
To visualize the results we calculated "good_customer_rate" which is the proportion of good customers, i.e. cnt_status_good divided by total of all statuses.  
...
ggplot(data = df.cc,
  ...
  geom_point(col = "blue", 
             size = 1)
```
For the GLM binomial model, there is slight evidence that the predictors have an influence. When visualizing the results we can see that we have very few observations at the upper end of the amt_income_total scale. This makes us doubt the validity of the results. Interestingly, the predicted variables show a downward trend, although the observations with the highest income are between 75% and 100% of the dependent variable good_customer_rate.

## Conclusion (Binomial Model)
In this model, we mainly concentrated on the continuous variables to see if their relevance for the new response variable was relevant. Our findings showed that this was not true for all but the amt_income_total variable.

# GAM Model
For the GAM model we use the response variable percent_good, which is the percentage of good customers in our data. 
Note: graphical analysis not repeated as this response variable was used in the linear model as well. 

## Fit GAM Models
We start again with a basic model in which amt_income_total is the predictor. The p-value for amt_income_total is not significant. We try a few models with more variables and show only the summary output for the most complex model.
...
summary(gam.cc.3)
```
Here, we see that the edf of the smooth terms amt_income_total and age_years are 1, which mean that the effect on the response variable should be linear. For the smooth term work_years the edf is higher than 4 indicating that the effect on the response variable is a more complex polynomial. The p-values of all smooth terms show no significance, and tell us that there is no evidence that the effect differs from zero.  
...
## Visualization smooth terms
If the smooth terms were significant, i.e. whether a higher level of complexity is required, it would be evident in the visualization, so we present the smooth terms here to confirm our findings from the summary out above. 
...
## Cross Validation
Here we perform cross-validation on the GAM models using a 10-fold cross-validation approach. The R^2 value was calculated for each model during each run to compare the mean value for the three models. 
```{r gam-cv, message=FALSE}
set.seed(101)
...
r.squared.results.gam <- lapply (folds, function(x){
  ...
  #gam.cc.1
  gam.cc.1.train <- gam(formula = formula(gam.cc.1), data = df.cc.train)
  predict.gam.cc.1.test <- predict(gam.cc.1.train, newdata = df.cc.test)
  r.squared.gam.cc.1 <- cor(predict.gam.cc.1.test, df.cc.test$percent_good)^2
  
  # gam.cc.2
  gam.cc.2.train <- gam(formula = formula(gam.cc.2), data = df.cc.train)
  predict.gam.cc.2.test <- predict(gam.cc.2.train, newdata = df.cc.test)
  r.squared.gam.cc.2 <- cor(predict.gam.cc.2.test, df.cc.test$percent_good)^2
  
  # gam.cc.3
  gam.cc.3.train <- gam(formula = formula(gam.cc.3), data = df.cc.train)
  predict.gam.cc.3.test <- predict(gam.cc.3.train, newdata = df.cc.test)
  r.squared.gam.cc.3 <- cor(predict.gam.cc.3.test, df.cc.test$percent_good)^2
...
```{r Comparison of the R squared, echo=FALSE, message=FALSE}
df.r.squared.gam <- data.table::transpose(data.frame(r.squared.results.gam))
colnames(df.r.squared.gam) <- c("gam.cc.1", "gam.cc.2", "gam.cc.3")
rownames(df.r.squared.gam) <- colnames(data.frame(r.squared.results.gam))

mean.table <- c(mean(df.r.squared.gam$gam.1), mean(df.r.squared.gam$gam.2), mean(df.r.squared.gam$gam.3))
names(mean.table) <- c("gam.cc.1", "gam.cc.2", "gam.cc.3")
mean.table
```
When we calculate the mean of the R^2 values produced by the 10-fold cross-validation, we can see that gam.cc.3 has the highest mean R^2, suggesting this best explains the data. 

```{r Visualization of the R squared, echo=FALSE, message=FALSE}
boxplot(df.r.squared.gam)
```
The boxplot shows us that the gam.cc.3 model also has the greatest spread, indicating that the R^2 value is not as reliable. We can also see that the R^2 of the simplest model with the smooth term amt_income_total has the highest median and the lowest spread.  
  
When we compare the adjusted R^2 from the summary outputs, to include the complexity of the models, it shows that the most complex model has the highest value. Ultimately, we would still tend to use the simpler model, as it contains far fewer variables and the mean values of the R^2 do not differ a lot between all models. Overall, it should be noted that R^2, respectively adjusted R^2, for all models are very low suggesting very limited predictive power.

## Conclusion (GAM Model)
We wanted to do cross validation on the GAM Model since we believe that this model is sensitive to overfitting due to its flexibility. However we are unsure in our case, if R^2 and adjusted R^2 was the right choice for measuring the goodness of fit.

# Support Vector Machine
In the following chapter, a Support Vector Machine is used to predict whether a customer is a good 
or bad customer. Here we use again the definition that a good customer repays loans more than 50% of the time within 60 days.

## Graphical Analysis
To first get an overview of potentially good predictor combinations (e.g. where we can see a 
threshold between good and bad customers), we plot different predictors against the response variable customer_status_good. 
Note: we include only two plots for simplicity.
...
```
The graphical analysis does not show clear thresholds from different predictor variables for customer_status_good.

## Data Preparation
Since the SVM method cannot handle categorical variables directly, these are transformed to dummy variables:
...
## Fit SVM Models
For making predictions, we split the data. We use 70% of the data to train the SVM. Using (almost) all potential predictors, we get an accuracy of around 83.8%. To see if we get better results, we reduce the number of predictors. For example:
...
With only four predictors, we get the same result as the model that included all predictors. No matter which combination was used for the model, we were unable to produce a significantly better accuracy for our predictions.  

Below, we show a sample of the tested combinations and their respective predictive accuracy:
...
What is evident from this table is that using only a single predictor (amt_income_total), we are able to achieve a similar level of accuracy to the model with all predictors included.

## Conclusion (SVM)
The accuracy of 83.8% is not just a random percentage number, it is close to the percentage of the good customers in the prediction-testing set. The predictors are too weak, to differentiate between good and bad customers, or in other words: good and bad customers are to well spread over the different predictors, which can be seen graphically:
This can be easily seen graphically:

```{r svm-plot, echo=FALSE}
plot(test_svm, train, amt_income_total ~ work_years)
```
The SVM sees only potential good customers and cannot distinguish from bad customers. The available predictors don't correspond well to the response variable, therefore an SVM model with the predictors at hand is not a good choice.
...
The accuracy indicates a good score (`r sum(diag(cm_nn))/sum(sum(cm_nn))`). Simply predicting all customers as good customers would lead to a even higher accuracy (approx. 0.84).  

In contrast, the model has difficulty identifying bad customers. It falsely identifies `r cm_nn[1, 1] + cm_nn[1, 2]` bad customers, despite most of them in fact are good customers. Out of the `r cm_nn[1, 1] + cm_nn[2, 1]` 'real' bad customers  the model identifies only `r cm_nn[1, 1]`.

## Tuning the neurons
This function helps to tune the number of neurons for the model. 
```{r echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, include=TRUE}
...
```
However, we can see that there is no model that gives a high accuracy and is able to identify bad customers well.

## Importance of the predictors
This table shows the importance of the different predictors (list of top 10 out of 41).
```{r}
...
```
This output should not be overinterpreted as the model itself is not suitable to identify bad customers.

## Conclusion (Neural Networks)
The research team invested a lot of time to find models, neural network designs and tuning techniques, which are stable and could increase the accuracy over 84% (which is equal to the share of good customers). Such a model could not be found. While neural networks are flexible with many variants and generally are able to find complex patterns, this was not the case with our dataset (especially predictors).

Another question, which the analysis was unable to answer, was the definition of a "good" accuracy. The client could already benefit by finding a few "true" bad customers. A good customer would repay loans including interest rate and on the other hand the company would lose a lot of money if a bad customer is unable to repay the loan. It could make sense not to rely only on accuracy in this case and focus on identifying bad customers instead.

Lastly, the practical implementation of the model needs to be discussed. The model would help to identify potential "bad customers" already at the application process and allow the company to decline applicants before a credit card is issued. On the other hand, using "black box" methods like neural networks is not always suitable for such a decision process. The company would not be able to give reasons why the application of a customer was denied. 

# Optimization
The distinction between good and bad customers is a way to build up a relatively safe portfolio of customers, who repay their debts with interest. However, the defined threshold leaves room for optimization.  
...
Method <- c('LM','GLM-Poisson (quasi-poisson)','GLM-Binomial','GAM','SVM','ANN')

`Dependent variable` <- c('percent_good', 
                          'cnt_status_good',
                          'proportion of customer_status_good', 
                          'percent_good',
                          'customer_status_good', 
                          'customer_status_good')

Predictors <- c('occupation_type',
                'occupation_type', 
                'amt_income_total', 
                'amt_income_total',
                'amt_income_total + flag_own_realty',
                'gender+age+education+income_type+housing+family_status+cnt_fam_members+own_car+own_realty+log_amt_income_total+occupation_type')

`Measure of fit` <- c('R^2 and adjusted R^2', 
                      'p-value using anova(fit1, fit2)', 
                      'N/A', 

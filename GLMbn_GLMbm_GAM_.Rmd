...
When interaction between all variables is included, it is interesting to see that it plays a significant role, e.g. there is strong evidence of interaction for amt_income_total and work_years as well as for amt_income_total and cnt_children. P-values of interaction between other variables show significance too. Also the pairs plot shows that there is interaction between different variables.
...
ggplot(data = df.cc,
  ...
  geom_point(col = "blue", 
             size = 1)
```
For the GLM binomial model there is in general little evidence that the predictors have significant influence. When visualizing the results we can see that we have not very much observation at the upper end of the amt_income_total scale. This makes us doubt the validity of the results. It makes little sense that although those observations with the highest income are between 75% and 100% of the dependent variable good_customer_rate, the predicted variables show a downward trend.


### GAM Model
For the GAM model we use the response variable percent_good, which is the percentage of good customers in our data. We start again with a basic model in which amt_income_total is the predictor.
...
```
The p-value for amt_income_total is not significant. We add more variables to the model, code_gender, age_years.
```{r GAM 2 to 4, include=FALSE, message=FALSE}
gam.cc.2 <- update(gam.cc.1, . ~ . + occupation_type)
summary(gam.cc.2)

gam.cc.3 <- update(gam.cc.2, . ~ . + s(age_years))
summary(gam.cc.3)
```
Since the edf is 1 for all this variables, there is no evidence that one of the variables has a non-linear effect on the response variable (models not shown). The p-values also do not show significance.

We try a more complex model with all the variables.
```{r GAM complex model, message=FALSE}
gam.cc.4 <- gam(percent_good ~ amt_income_total + code_gender + 
                  age_years + work_years + name_education_type + name_income_type + 
                  name_housing_type + cnt_children + flag_own_car + flag_own_realty + 
                  occupation_type, 
                data = df.cc)
summary(gam.cc.4)
```

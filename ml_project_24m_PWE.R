...
#### GAM ####
# respons variable: status or income?
# name_income-type is not taken into account, because occupation_type has in our opinion a better
# categorization. flag for mobile, phone and email is also not taken into account (not interesting)
# no name_housing_type
# no cnt_fam_members

pairs(amt_income_total ~ code_gender + age_years + name_education_type + name_family_status + 
        cnt_children + flag_own_car + flag_own_realty + work_years + occupation_type +
        month_balance,
      data = df.cc.sample,
      pch = ".",
      upper.panel = panel.smooth)

hist(df.cc.sample$amt_income_total) # log because amt_total_income is right skewed and is an amount
# hist(log(df.cc.sample$amt_income_total))
gam.income <- gam(log(amt_income_total) ~ code_gender + age_years + name_education_type +
                    name_family_status + cnt_children + flag_own_car + flag_own_realty +
                    work_years + occupation_type + month_balance,
                  data = df.cc.sample)

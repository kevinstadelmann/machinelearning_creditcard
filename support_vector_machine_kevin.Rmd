...
intrain <- createDataPartition(y = df.svm$customer_status_good, p=0.85, list = FALSE)
...
test_svm <- svm(customer_status_good ~ amt_income_total + work_years + cnt_children +cnt_fam_members,
                df.svm.training,
                kernel = "linear",
                scale = TRUE, cost = 10)

# make the prediction
test_pred <- predict(test_svm, df.svm.testing)

# compare the predicted data with the testdata subset
confusionMatrix(table(test_pred, df.svm.testing$customer_status_good))

# plot results
plot(test_svm, df.svm.training, df.svm$work_years ~ df.svm$age_years)

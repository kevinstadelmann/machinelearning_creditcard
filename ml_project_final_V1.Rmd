...
# GLM Binomial Model
For the GLM Binomial model we use the response variable proportion of good customers. Therefore we divide the customer status in the data set into good (1) and bad (0) customer count for creditworthiness. We would expect amt_income_total to have the strongest impact on the response variable, which is related to creditworthiness. We are also interested in seeing if variables such as age_years, work_years and cnt_children have a significant impact on the target variables.
...
# GAM Model
For the GAM model we use the response variable percent_good, which is the percentage of good customers in our data. We start again with a basic model in which amt_income_total is the predictor. The p-value for amt_income_total is not significant. We try a few models with more variables and show only the most complex model.
...
summary(gam.cc.3)
```
The edf of the smooth terms amt_income_total and age_years is 1, which means that the effect on the response variable should be linear. For the smooth term work_years the edf is higher than 4 which means that the effect on the response variable is a more complex polynomial. The p-values of all smooth terms show no significance. Hence, there is no evidence that the effect differs from zero.
...
names(mean.table) <- c("gam.1", "gam.2", "gam.3")
mean.table

boxplot(df.r.squared.gam)
```
When we calculate the mean of the R^2 values produced by the 10-fold cross-validation, we can see that gam.cc.3 has the highest mean R^2, suggesting this best explains the data. The boxplot shows us that there is also the greatest spread with gam.cc.3, indicating that the R^2 value is not as reliable. We can also see that the R^2 of the simplest model with the smooth term amt_income_total has the highest median and the lowest spread. However if we compare in the summary outputs the adjusted R squared of the most complex model has the highest value. We would still tend to use the simpler model, since it contains far fewer variables and the mean values of the R^2 do not differ a lot between all models. Overall, it should be noted that R^2, respectively R squared adjusted is very low suggesting very limited predictive power of these models.
...
Overall, the following predictors were to a certain point useful:
- amt_income_total
- flag_own_reality
- name_income_type

For further predictions attempts, we highly suggest to look for stronger predictor
variables.

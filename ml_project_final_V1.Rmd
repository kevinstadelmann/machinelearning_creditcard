...
`Dependent variable` <- c('percentage_good', 'cnt_status_good','', '','customer_status_good', 'customer_status_good')
...
Performance <- c('R^2 and adjusted R^2', 'p-value using anova(fit1, fit2)', '', '', 'Accuracy in %', 'Accuracy in %')

table <- data.frame(Method, `Dependent variable`, Predictors, Performance)
table
```

Personal note:

We had high expectations on the data set and the outcome of our models. However, reality hits you hard sometimes. Two things we would like to mention here:
- Data preparation: Half of the effort was data preparation and discussion how to define the response variable(s). At one point we agreed on one definition. Choosing another definition of what a bad customer looks like, might change the outcome of our models drastically.
- Data set: Looking back, we would choose a data set with less categorical / binomial data. Generally speaking, we made the experience that categorical variables are harder to interpret and to more difficult to use them as good predictors.
